...
import oms3.Notification.ExceptionEvent;
...
public class Sim extends AbstractSimulation {
	...
	public Object run() throws Exception {
		...
		if (!success) {
			throw new ComponentException(
					"There are Parameter problems. Simulation exits.");
		}
		...
		boolean adjusted = ComponentAccess.adjustOutputPath(lastFolder, comp,
				log);
					...
					c.addListener(new Listener() {
						...
						public void notice(Type arg0, EventObject arg1) {
							if (arg0 == Type.OUT) {
								...
								Object v = e.getValue();
								if (v == null) {
									log.fine("'null' output from "
											+ e.getAccess().toString());
								}
							}
						}
					});
	...
	void document(File file) throws Exception {
		...
		Documents.db5Sim(file, // output file
				...
				getName(), locale);// simulation name
	...
	public String digest() throws Exception {
		...
		b.append(digest(res) + '\n');
		Collection<Class<?>> c = Components.internalComponents(model
